...
class LyricPainter extends CustomPainter with ChangeNotifier {
  ...
  LyricPainter(this.lyric, this.curLine) {
    linePaint = Paint()
      ..color = Colors.white12
      ..strokeWidth = ScreenUtil().setWidth(1);
    lyricPaints.addAll(lyric
        .map((l) => TextPainter(
        text: TextSpan(text: l.lyric, style: TextStyle(fontSize: 16, color: Colors.grey)),
    ...
    for (int i = 0; i < lyric.length; i++) {
      if (y > size.height || y < (0 - lyricPaints[i].height / 2)) {
        ...
        if (curLine == i) {
          // 如果是当前行
          lyricPaints[i].text =
              TextSpan(text: lyric[i].lyric, style: TextStyle(fontSize: 16, color: Colors.white));
          ...
          // 如果是拖动状态中的当前行
          lyricPaints[i].text =
              TextSpan(text: lyric[i].lyric, style: TextStyle(fontSize: 16, color: Colors.white70));
          lyricPaints[i].layout();
        } else {
          lyricPaints[i].text =
              TextSpan(text: lyric[i].lyric, style: TextStyle(fontSize: 16, color: Colors.grey));
          lyricPaints[i].layout();
        }
